...
if start_DeepMHC == "Y": 
    ...
    for piece in data:
        if "HLA" in piece and newline.shape[0] > 0:
            big_data = pd.concat([big_data,newline],axis=0,ignore_index=True)
            newline=pd.DataFrame()
    ...
    mhc_name = pd.DataFrame(nn_df.iloc[:,0])
    mhc_name.columns = ["mhc_name"]
    pseud_mhc = pd.DataFrame(nn_df.iloc[:,1])
    pseud_mhc.columns = ["pep_seq"]
    one = pd.DataFrame(nn_df.iloc[:,5])
    one.columns = ["binding_core_1"]
    one_s = pd.DataFrame(nn_df.iloc[:,6])
    one_s.columns = ["binding_core_score_1"]
    two = pd.DataFrame(nn_df.iloc[:,7])
    two.columns = ["binding_core_2"]
    two_s = pd.DataFrame(nn_df.iloc[:,8])
    two_s.columns = ["binding_core_score_2"]
    three = pd.DataFrame(nn_df.iloc[:,9])
    three.columns = ["binding_core_3"]
    three_s = pd.DataFrame(nn_df.iloc[:,10])
    three_s.columns = ["binding_core_score_3"]
    four = pd.DataFrame(nn_df.iloc[:,11])
    four.columns = ["binding_core_4"]
    four_s = pd.DataFrame(nn_df.iloc[:,12])
    four_s.columns = ["binding_core_score_4"]
    five = pd.DataFrame(nn_df.iloc[:,13])
    five.columns = ["binding_core_5"]
    five_s = pd.DataFrame(nn_df.iloc[:,14])
    five_s.columns = ["binding_core_score_5"]
    six = pd.DataFrame(nn_df.iloc[:,15])
    six.columns = ["binding_core_6"]
    six_s = pd.DataFrame(nn_df.iloc[:,16])
